...
inline int line_search_lewisoverton(
  ...
  if (!(stp > 0.0)) {
    // std::cout << "LBFGSERR_INVALIDPARAMETERS\r\n";
  ...
  if (0.0 < dginit) {
    // std::cout << "return LBFGSERR_INCREASEGRADIENT\r\n";
  ...
  while (true) {
    ...
    if (stp < stpmin) {
      ...
      // std::cout << "LBFGSERR_MINIMUMSTEP\r\n";
      return LBFGSERR_MINIMUMSTEP;
    }
    if (stp > stpmax) {
      if (touched) {
        /* The step is the maximum value. */
        // std::cout << "LBFGSERR_MAXIMUMSTEP\r\n";
  ...
  if (gnorm_inf / std::max(1.0, xnorm_inf) < param.g_epsilon) {
    ...
    */
    ...
    while (true) {
      ...
      // }
      // while(1){
      //     // std::cout<<"step * d"<<(step *
      //     d).transpose()<<std::endl; x = xp + step * d;
      //     /* Evaluate the function and gradient values. */
      //     double f = cd.proc_evaluate(cd.instance, x, g);
      //     if(g.norm()>1.0){
      //         g = g.normalized().eval()*1.0;
      //     }
      //     std::cout<<"fx: "<<fx<<"  f: "<<f<<"step:
      //     "<<step<<std::endl; if(f < fx || step <= step_min){
      //         fx = f;
      //         break;
      //     }
